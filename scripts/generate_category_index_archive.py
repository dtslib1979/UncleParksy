...
# -*- coding: utf-8 -*-
"""
Generate Archive-Style Index Files for Categories
Creates beautiful archive-style index.html files for each category
"""
...
CATEGORIES = {
    "system-configuration": {
        ...
        "subtitle": "시스템 설정과 구성의 기록들",
        "label": "System Config"
    },
    "thought-archaeology": {
        "title": "🏛️ Thought Archaeology",
        "subtitle": "생각의 고고학, 사유의 발굴지",
        "label": "Deep Thought"
    },
    "webappsbook-codex": {
        "title": "📚 WebAppsBook Codex",
        "subtitle": "웹앱 개발서의 코덱스",
        "label": "WebApps Code"
    },
    "webappsbookcast": {
        "title": "🎙️ WebAppsbookcast",
        "subtitle": "웹앱북 팟캐스트의 기록들",
        "label": "Podcast"
    },
    "writers-path": {
        "title": "✍️ Writers Path",
        "subtitle": "글쓰기의 길, 작가의 여정",
        "label": "Writing Story"
    }
}

def get_archive_template(category_name, config):
    """Get the archive template with category-specific content"""
    return f'''<!doctype html>
<html lang="ko">
<head>
<meta charset="utf-8" />
<meta name="viewport" content="width=device-width,initial-scale=1" />
<title>{config["title"].replace("🏛️ ", "").replace("⚙️ ", "").replace("📚 ", "").replace("🎙️ ", "").replace("✍️ ", "")} — UncleParksy Category</title>
<meta name="description" content="{config["subtitle"]} from the EduArt Engineer's digital archive" />
...
.treasure-card{{
  ...
  /* Random rotation and offset for "삐뚤빼뚤" effect */
...
<body>
  ...
  <header class="archive-header">
    <h1 class="archive-title">{config["title"]}</h1>
    <p class="archive-subtitle">{config["subtitle"]}</p>
  </header>

  <!-- 📊 File Counter Display -->
  <div class="file-counter">
    <p class="counter-text">
      총 <span class="counter-number" id="file-count">0</span>개의 보물 이야기가 보관되어 있습니다
    </p>
  </div>

  <!-- Controls -->
  <nav class="archive-controls">
    <a href="../../index.html" class="back-link">← 그리모어로 돌아가기</a>
    
    <div class="sort-controls">
      <button class="sort-btn active" data-sort="random">🎲 랜덤</button>
  ...
  <main class="treasure-container">
    <div class="scroll-viewer">
      ...
      <div id="error" class="error-message" style="display:none">
        ❌ 보물 목록을 불러올 수 없습니다. 나중에 다시 시도해주세요.
      </div>
      ...
      <div id="content" style="display:none">
        ...
        <div class="pagination" id="pagination">
          <button class="page-btn" id="prev-btn">이전</button>
          <div class="page-dots" id="page-dots"></div>
          <div class="page-info" id="page-info"></div>
          <button class="page-btn" id="next-btn">다음</button>
        </div>
      </div>
    </div>
  </main>
  ...
  <script>
    ...
    // 🎯 Load category manifest.json
    async function loadManifest() {{
      try {{
        console.log('📡 Fetching {category_name} manifest...');
        const response = await fetch('../../assets/{category_name}-manifest.json');
        ...
        console.log('🎉 {config["title"]} loaded successfully!');
        ...
        errorDiv.innerHTML = `
          ❌ 보물 목록을 불러올 수 없습니다.<br>
          <small style="opacity:0.7">오류: ${{error.message}}</small><br>
          <small style="opacity:0.7">경로: ../../assets/{category_name}-manifest.json</small>
        `;
      }}
    }}
    ...
    function updateFileCounter(count) {{
      ...
      if (counterElement) {{
        ...
        console.log(`📊 Counter updated: ${{count}} files`);
      }}
    }}
    ...
    function createTreasureCard(item, index) {{
      ...
      // Path handling for category files
      let cleanPath = item.path;
      if (cleanPath.startsWith('category/{category_name}/')) {{
        cleanPath = cleanPath.replace('category/{category_name}/', './');
      }}
      card.href = cleanPath;
      
      console.log(`🔗 Card link: ${{card.href}} (원본: ${{item.path}})`);
      
      // Random rotation and translation for "삐뚤빼뚤" effect
...
def main():
    """Generate all remaining category index files"""
    ...
    for category_name, config in CATEGORIES.items():
        ...
        print(f"📝 Generating index.html for {category_name}")
        
        # Generate the HTML content
        html_content = get_archive_template(category_name, config)
    ...
    print("🎉 All category index files generated successfully!")
    return 0
